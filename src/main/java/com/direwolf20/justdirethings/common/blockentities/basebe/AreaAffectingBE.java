...
public interface AreaAffectingBE {
	...
	default AABB getAABBOffsetOnly(BlockPos relativePos) {
		...
		return new AABB(relativePos.getX() + xOffset, relativePos.getY() + yOffset, relativePos.getZ() + zOffset,
				relativePos.getX() + xOffset
						+ (getAreaAffectingData().xRadius != Math.floor(getAreaAffectingData().xRadius) ? 0.5 : 1),
				relativePos.getY() + yOffset
						+ (getAreaAffectingData().yRadius != Math.floor(getAreaAffectingData().yRadius) ? 0.5 : 1),
				relativePos.getZ() + zOffset
						+ (getAreaAffectingData().zRadius != Math.floor(getAreaAffectingData().zRadius) ? 0.5 : 1));
	}
	...
	default AABB getAABB(BlockPos relativePos) {
		if (getAreaAffectingData().area == null) {
			...
			if (getAreaAffectingData().zRadius != Math.floor(getAreaAffectingData().zRadius))
				zOffset = zOffset + 0.5;
			getAreaAffectingData().area = new AABB(relativePos.getX() + xOffset, relativePos.getY() + yOffset,
					relativePos.getZ() + zOffset, relativePos.getX() + xOffset + 1, relativePos.getY() + yOffset + 1,
					relativePos.getZ() + zOffset + 1).inflate(getAreaAffectingData().xRadius,
							getAreaAffectingData().yRadius, getAreaAffectingData().zRadius);
		}
