...
public class ClickerT2Screen extends BaseMachineScreen<ClickerT2Container> {
	...
	public void init() {
		...
		addRenderableWidget(
				ToggleButtonFactory.CLICKTARGETBUTTON(getGuiLeft() + 44, topSectionTop + 62, clickTarget, b -> {
					clickTarget = ((ToggleButton) b).getTexturePosition();
					saveSettings();
				}));

		addRenderableWidget(
				ToggleButtonFactory.LEFTRIGHTCLICKBUTTON(getGuiLeft() + 44, topSectionTop + 44, clickType, b -> {
		...
		maxHoldTicksButton = new NumberButton(getGuiLeft() + 64, topSectionTop + 46, 24, 12, maxHoldTicks, 1, 1200,
		...
		addRenderableWidget(
				ToggleButtonFactory.SHOWFAKEPLAYERBUTTON(getGuiLeft() + 8, topSectionTop + 44, showFakePlayer, b -> {
