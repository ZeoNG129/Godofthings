...
public class ClickerT1Screen extends BaseMachineScreen<ClickerT1Container> {
	...
	public void addTickSpeedButton() {
		...
		if (clickType == 2) {
			tickSpeedButton = ToggleButtonFactory.TICKSPEEDBUTTON(getGuiLeft() + 144, topSectionTop + 40, tickSpeed,
					maxHoldTicks + 1, b -> {
						tickSpeed = ((NumberButton) b).getValue(); // The value is updated in the mouseClicked method
																	// below
	...
	public void init() {
		super.init();
		addRenderableWidget(
				ToggleButtonFactory.DIRECTIONBUTTON(getGuiLeft() + 122, topSectionTop + 38, direction, b -> {
					direction = ((ToggleButton) b).getTexturePosition();
					PacketHandler.CHANNEL.sendToServer(new DirectionSettingPayload(direction));
				}));

		addRenderableWidget(
				ToggleButtonFactory.CLICKTARGETBUTTON(getGuiLeft() + 56, topSectionTop + 38, clickTarget, b -> {
					clickTarget = ((ToggleButton) b).getTexturePosition();
					saveSettings();
				}));

		addRenderableWidget(
				ToggleButtonFactory.LEFTRIGHTCLICKBUTTON(getGuiLeft() + 38, topSectionTop + 38, clickType, b -> {
		...
		maxHoldTicksButton = new NumberButton(getGuiLeft() + 34, topSectionTop + 55, 24, 12, maxHoldTicks, 1, 1200,
				Component.translatable("justdirethings.screen.click-hold-for"), b -> {
					maxHoldTicks = ((NumberButton) b).getValue(); // The value is updated in the mouseClicked method
																	// below
					if (clickType == 2) {
						tickSpeed = Math.max(tickSpeed, maxHoldTicks + 1);
						PacketHandler.CHANNEL.sendToServer(new TickSpeedPayload(tickSpeed));
		...
		addRenderableWidget(
				ToggleButtonFactory.SHOWFAKEPLAYERBUTTON(getGuiLeft() + 2, topSectionTop + 38, showFakePlayer, b -> {
	...
	public void saveSettings() {
		...
		PacketHandler.CHANNEL
				.sendToServer(new ClickerPayload(clickType, clickTarget, sneaking, showFakePlayer, maxHoldTicks));
	}
}
