...
public class BlockSwapperT1Screen extends BaseMachineScreen<BlockSwapperT1Container> {
	...
	protected final ResourceLocation INACTIVE = new ResourceLocation(JustDireThings.MODID,
			"textures/gui/buttons/inactive.png");
	...
	public void init() {
		super.init();
		addRenderableWidget(
				ToggleButtonFactory.SWAPPERENTITYBUTTON(getGuiLeft() + 106, topSectionTop + 38, swap_entity_type, b -> {
					swap_entity_type = ((ToggleButton) b).getTexturePosition();
					saveSettings();
				}));
		addRenderableWidget(
				ToggleButtonFactory.SWAPPERBLOCKBUTTON(getGuiLeft() + 88, topSectionTop + 38, swapBlocks ? 0 : 1, b -> {
	...
	public void addRedstoneButtons() {
		addRenderableWidget(ToggleButtonFactory.REDSTONEBUTTON(getGuiLeft() + 124, topSectionTop + 38,
	...
	protected void renderTooltip(GuiGraphics pGuiGraphics, int pX, int pY) {
		...
		if (MiscTools.inBounds(activeX, activeY, 16, 16, pX, pY)) {
			boundTo = be.boundTo;
			if (boundTo != null) {
				ChatFormatting chatFormatting = container.getPartnerExists() == 1
						? ChatFormatting.BLUE
						: ChatFormatting.DARK_RED;
				String key = container.getPartnerExists() == 1
						? "justdirethings.boundto"
						: "justdirethings.boundto-missing";
				pGuiGraphics
						.renderTooltip(this.font,
								Language.getInstance()
										.getVisualOrder(
												Arrays.asList(
														Component
																.translatable(key,
																		Component.translatable(boundTo.dimension()
																				.location().getPath()),
																		"[" + boundTo.pos().toShortString() + "]")
																.withStyle(chatFormatting))),
								pX, pY);
			} else {
				ChatFormatting chatFormatting = ChatFormatting.DARK_RED;
				pGuiGraphics.renderTooltip(this.font,
						Language.getInstance().getVisualOrder(Arrays.asList(
