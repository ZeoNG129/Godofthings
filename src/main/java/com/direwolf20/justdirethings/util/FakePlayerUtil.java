...
/**
 * This class completely and shamelessly stolen from Shadows from his mod Click
 * Machine :)
 */
public class FakePlayerUtil {
	 ...
	 *            The player.
	 * @param pos
	 *            The position of the using tile entity.
	 * @param direction
	 *            The direction to use in.
	 * @param toHold
	 *            The stack the player will be using. Should probably come from an
	 *            ItemStackHandler or similar.
	 */
	public static void setupFakePlayerForUse(UsefulFakePlayer player, Vec3 pos, Direction direction, ItemStack toHold,
			boolean sneaking) {
	 ...
	 *            ItemStackHandler or similar.
	 */
	public static void setupFakePlayerForUse(UsefulFakePlayer player, BlockPos pos, Direction direction,
	 ...
	 */
	public static void setupFakePlayerForUse(UsefulFakePlayer player, BlockPos pos, Direction direction,
			Vec3 entityPosition, ItemStack toHold, boolean sneaking) {
		player.getInventory().items.set(player.getInventory().selected, toHold);
		float xRot = direction == Direction.DOWN ? 90 : direction == Direction.UP ? -90 : 0;
		player.setXRot(xRot);
		player.setYRot(direction.toYRot());
		...
		float pitch = (float) Math
				.toDegrees(-Math.atan2(toEntity.y, Math.sqrt(toEntity.x * toEntity.x + toEntity.z * toEntity.z)));
	 ...
	 *            The player.
	 * @param oldStack
	 *            The previous stack, from before use.
	 */
	public static void cleanupFakePlayerFromUse(UsefulFakePlayer player, ItemStack oldStack) {
		...
		// //Disabed for now, since I have no plans to give these players anything
		// besides a mainhand
		player.setShiftKeyDown(false);
		player.setReach(player.getAttributeValue(ForgeMod.BLOCK_REACH.get()));
	}

	public static FakePlayerResult clickEntityInDirection(UsefulFakePlayer player, Level world, LivingEntity entity,
			int clickType, int maxHold) {
	 ...
	 *            The player.
	 * @param world
	 *            The world of the calling tile entity. It may be a bad idea to use
	 *            {FakePlayer#getEntityWorld()}.
	 * @param clickType
	 *            Right or Left click. 0 = right. 1 = left.
	 * @return The remainder of whatever the player was holding. This should be set
	 *         back into the tile's stack handler or similar.
	 */
	public static FakePlayerResult clickBlockInDirection(UsefulFakePlayer player, Level world, int clickType,
			int maxHold) {
		...
		if (toUse.getType() == HitResult.Type.BLOCK) {
			...
			if (!state.isAir()) {
				if (clickType == 0) {
					InteractionResult type = player.gameMode.useItemOn(player, world, itemstack,
							InteractionHand.MAIN_HAND, (BlockHitResult) toUse);
					if (type == InteractionResult.SUCCESS || type == InteractionResult.CONSUME)
						return new FakePlayerResult(InteractionResult.SUCCESS, player.getMainHandItem());
				} else {
					player.gameMode.handleBlockBreakAction(blockpos,
							ServerboundPlayerActionPacket.Action.START_DESTROY_BLOCK,
							((BlockHitResult) toUse).getDirection(), player.level().getMaxBuildHeight(), 0);
					return new FakePlayerResult(InteractionResult.SUCCESS, player.getMainHandItem());
				}
			}
		}

		if (toUse.getType() == HitResult.Type.MISS) { // Since we check for air before entering the method, there must
														// be a block there, so try clicking it anyway
			if (clickType == 0) {
				InteractionResult type = player.gameMode.useItemOn(player, world, itemstack, InteractionHand.MAIN_HAND,
						(BlockHitResult) toUse);
				if (type == InteractionResult.SUCCESS || type == InteractionResult.CONSUME)
					return new FakePlayerResult(InteractionResult.SUCCESS, player.getMainHandItem());
			}
		}
		...
		/*
		 * if (itemstack.isEmpty() && (toUse == null || toUse.getType() ==
		 * HitResult.Type.MISS)) { CommonHooks.onEmptyClick(player,
		 * InteractionHand.MAIN_HAND); //Empty Hand Click i assume? return new
		 * FakePlayerResult(InteractionResult.SUCCESS, player.getMainHandItem()); }
		 */
		if (!itemstack.isEmpty()) {
			InteractionResult type = player.gameMode.useItem(player, world, itemstack, InteractionHand.MAIN_HAND); // Uses
																													// the
																													// item
																													// by
																													// itself
	...
	public static FakePlayerResult rightClickAirInDirection(UsefulFakePlayer player, Level world, int clickType,
			int maxHold) {
		HitResult toUse = rayTraceBlock(player, world, player.getReach()); // Longer reach so it can connect with
																			// adjacent blocks to interact with them
		if (toUse == null)
		 ...
		 * if (itemstack.isEmpty() && (toUse == null || toUse.getType() ==
		 * HitResult.Type.MISS)) CommonHooks.onEmptyClick(player,
		 * InteractionHand.MAIN_HAND); //Empty Hand Click i assume?
		 */
		if (!itemstack.isEmpty()) {
			InteractionResult type = player.gameMode.useItem(player, world, itemstack, InteractionHand.MAIN_HAND); // Uses
																													// the
	 ...
	 *            {FakePlayer#getEntityWorld()}.
	 * @param pos
	 *            The pos of the calling tile entity.
	 * @param side
	 *            The direction to attack in.
	 * @param sourceState
	 *            The state of the calling tile entity, so we don't click ourselves.
	 * @return The remainder of whatever the player was holding. This should be set
	 *         back into the tile's stack handler or similar.
	 */
	public static ItemStack leftClickInDirection(UsefulFakePlayer player, Level world, BlockPos pos, Direction side,
			...
			for (int i = 1; i <= 5; i++) {
				...
				if (state != sourceState && !state.isAir()) {
					player.gameMode.handleBlockBreakAction(pos.relative(side, i),
							ServerboundPlayerActionPacket.Action.START_DESTROY_BLOCK, side.getOpposite(),
							player.level().getMaxBuildHeight(), 0);
					return player.getMainHandItem();
				}
			}
		}
	 ...
	 *            The player.
	 * @param world
	 *            The world of the calling tile entity.
	...
	public static HitResult traceEntities(UsefulFakePlayer player, Vec3 base, Vec3 target, Level world) {
		...
		List<Entity> list = world.getEntities(player, search,
	 ...
	 * @param result
	 *            The actual ray trace result, only necessary if using
	 *            {CUseEntityPacket.Action#INTERACT_AT}
	 * @param action
	 *            The type of interaction to perform.
	 * @return If the entity was used.
	 */
	public static boolean processUseEntity(UsefulFakePlayer player, Level world, Entity entity,
			@Nullable HitResult result, InteractionType action) {
		if (entity != null) {
			if (player.distanceToSqr(entity) < 36) {
				if (action == InteractionType.INTERACT) {
					...
					if (ForgeHooks.onInteractEntityAt(player, entity, result.getLocation(),
							InteractionHand.MAIN_HAND) != null)
						return false;
					return entity.interactAt(player, result.getLocation(),
	...
	public static HitResult rayTrace(UsefulFakePlayer player, Level level, double reachDist) {
		...
		HitResult trace = level
				.clip(new ClipContext(base, target, ClipContext.Block.OUTLINE, ClipContext.Fluid.SOURCE_ONLY, player));
