package com.godofthings.toolbelt.client.radial;
...
public class GenericRadialMenu
{
    ...
    private void processMouse(int mouseX, int mouseY)
    {
        ...
        for (int i = 0; i < numItems; i++)
        {
            ...
            if (a >= s && a < e && d >= radiusIn && (d < radiusOut || ToolBeltConfig.clipMouseToCircle || ToolBeltConfig.allowClickOutsideBounds))
            {
                hovered = i;
                break;
            }
        ...
        if (ToolBeltConfig.clipMouseToCircle)
        {
            Window mainWindow = minecraft.getWindow();
