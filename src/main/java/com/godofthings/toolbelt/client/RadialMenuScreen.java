...
import com.godofthings.toolbelt.client.radial.*;
...
public class RadialMenuScreen extends Screen
{
    ...
    public RadialMenuScreen(BeltFinder.BeltGetter getter)
    {
        ...
        insertMenuItem = new TextRadialMenuItem(menu, Component.translatable("text.godofthings.toolbelt.insert"))
        {
            @Override
    ...
    public void tick()
    {
        ...
        {
            if (ToolBeltConfig.releaseToSwap)
            {
                processClick(false);
            }
            else
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks)
    {
        ...
        ItemStack inHand = minecraft.player.getMainHandItem();
        if (ToolBeltConfig.isItemStackAllowed(inHand) && menu.isReady() && inventory != null)
        {
            if (needsRecheckStacks)
            {
                ...
                for (int i = 0; i < inventory.getSlots(); i++)
                {
                    ...
                    ItemStackRadialMenuItem item = new ItemStackRadialMenuItem(menu, i, inSlot, Component.translatable("text.godofthings.toolbelt.empty"))
                    {
                        @Override
                        public boolean onClick()
                        {
                    ...
                    item.setVisible(inSlot.getCount() > 0 || ToolBeltConfig.displayEmptySlots);
                    if (inHand.getCount() > 0)
                    {
                        if (inSlot.getCount() > 0)
                            item.setCentralText(Component.translatable("text.godofthings.toolbelt.swap"));
                        else
                            item.setCentralText(Component.translatable("text.godofthings.toolbelt.insert"));
                    }
                    else
                    {
                        if (inSlot.getCount() > 0)
                            item.setCentralText(Component.translatable("text.godofthings.toolbelt.extract"));
                        else
                            item.setCentralText(Component.translatable("text.godofthings.toolbelt.empty"));
                    }
            ...
            if (!ToolBeltConfig.displayEmptySlots && !cachedMenuItems.stream().allMatch(RadialMenuItem::isVisible) && inHand.getCount() > 0)
            {
                hasAddButton = true;
            }
            ...
            {
                menu.setCentralText(Component.translatable("text.godofthings.toolbelt.empty"));
            }
            else
            {
    ...
    {
        ItemStack inHand = minecraft.player.getMainHandItem();
        if (!ToolBeltConfig.isItemStackAllowed(inHand) || !menu.isReady())
            return false;
