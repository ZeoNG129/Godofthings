...
public class ModeWheelScreen extends Screen
{
        ...
        for (ToolMode mode : ToolMode.values())
        {
            if (mode != ToolMode.CHAIN_MINING)
            {
                ...
                if (mode == ToolMode.OMNITOOL_MODE)
                {
                    ...
                    // 1.20.1 为 ForgeRegistries.ITEMS.containsKey，NeoForge 1.21.1 改用原版 BuiltInRegistries
                    shouldAddMode = net.minecraft.core.registries.BuiltInRegistries.ITEM.containsKey(omnitoolId);
                }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks)
    {
        ...
        // 1.20.1 为 minecraft.getFrameTime()（1.21.1 已删除），改用 getTimer().getGameTimeDeltaPartialTick(true)
        float currTick = minecraft.getTimer().getGameTimeDeltaPartialTick(true);
        ...
        // 1.21.1：Tesselator 不再暴露 getBuilder()，改为 tessellator.begin(mode, format) 直接返回 BufferBuilder；
        // BufferBuilder.end() 已删除，改用 buildOrThrow() 返回 MeshData
                                  ...
                                  int sliceCount, float animProgress)
    {
        ...
        for (int i = 0; i < sliceCount; i++)
        {
            ...
            // 1.21.1：vertex().color().endVertex() → addVertex().setColor()
